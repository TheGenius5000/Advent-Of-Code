...
visited = [[[[False for _ in range(4, 11)] for _ in directions_as_num.keys()] for _ in range(len(losses[0]))] for _ in range(len(losses))]
priority_queue = [] 
heapq.heappush(priority_queue, State(0, 0, None, 0, 0, 0, 0))
...
while True:
  ...
  if current_state.dir_amount != 0:
    visited[current_state.y][current_state.x][directions_as_num[current_state.direction]][current_state.dir_amount-4] = True
  for direction, (dir_x, dir_y) in directions.items():
    ...
    if direction != current_state.direction or current_state.direction == None:
      new_dir = 4
    else:
      new_dir = 1
    for _ in range(new_dir):
      next_x += dir_x
      next_y += dir_y
    if any([next_x < 0, next_y < 0, next_x > Stuff.end[0], next_y > Stuff.end[1]]): continue
    new_dir_amount = 1+current_state.dir_amount if direction == current_state.direction else 4
    if new_dir_amount > 10 or visited[next_y][next_x][directions_as_num[direction]][new_dir_amount-4]: continue
    h_n = Stuff.get_heuristic(next_x, next_y)
    g_n = current_state.total_cost
    toadd_x = current_state.x
    toadd_y = current_state.y
    for _ in range(new_dir):
      toadd_x += dir_x
      toadd_y += dir_y
      g_n += losses[toadd_y][toadd_x]
